...
class SmartFilter:
    ...
    ── Authoritative logic ──────────────────────────────────────────
    • Gate Keeper filters (GK) are HARD-LOCKED by `self.gatekeepers`
      – Any change to this list will automatically:
          → change PASSES (Y / 13)
          → change Weighted-Confidence calculation
    • Confidence  =  (weight_passed_GK / total_GK_weight) * 100
    • Score       =  count of TRUE filters across *all* 21 checks
    • analyze()   returns a **self-contained dict** incl. ready-to-send
      message so that main / alert modules do ZERO extra maths
    ────────────────────────────────────────────────────────────────
    """
    ...
    def analyze(self):
        ...
        # ---------- METRICS ----------------------------------------------------
        score            = sum(results.values())                    # TRUE across all filters
        ...
        confidence       = round(100 * passed_weight / total_gk_weight, 1) if total_gk_weight else 0.0
    ...
    def _check_liquidity_awareness(
        self, band_pct: float = 0.005, wall_factor: float = 5, history_len: int = 20
    ):
        bids, asks = self._fetch_order_book(depth=100)
    ...
    def _check_volatility_model(self,
        low_pct: float = 0.01, high_pct: float = 0.05, period: int = 14
    ):
