...
class PECEnhancedReporter:
    ...
    def _calculate_pnl_usd(self, entry_price: float, exit_price: float, direction: str) -> float:
        """Calculate P&L USD using notional position of $100 ($10 margin × 10x leverage)"""
        try:
    ...
    def _generate_detailed_signal_list(self):
        ...
        detail_lines.append("📋 DETAILED SIGNAL LIST: FIXED POSITION SIZE $10, LEVERAGE 10x, NOTIONAL $100")
        ...
        for signal in sorted(self.signals, key=lambda s: s.get('fired_time_utc', ''), reverse=True):
            ...
            # P&L - RECALCULATE using notional position of $100 ($10 margin × 10x leverage)
    ...
    def generate_report(self):
        ...
        for conf_level, signals_list, label in [
            ...
            # RECALCULATE P&L using notional position of $100 ($10 margin × 10x leverage)
            pnl = 0.0
            for s in signals_list:
