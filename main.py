...
TOKENS = [
    ...
    "ELDE-USDT",
    "MAGIC-USDT",
    "FUN-USDT",       # perpetual on KuCoin swap
    "ACTSOL-USDT",    # corrected perpetual symbol
]
...
def check_symbol_availability():
    ...
    for tok in TOKENS:
        ...
        print(f"{tok:12} → Spot? {ok_spot}  Swap? {ok_swap}  Binance? {ok_bn}")
    print("---------------------------------")
...
def run():
    while True:
        ...
        for symbol in TOKENS:
            # Pre-fetch both 3min and 5min data
            ...
            if (key3 not in last_sent) or ((now - last_sent[key3]) >= COOLDOWN["3min"]):
                ...
                # Gate: require 5min volume spike (>1.5× 10-bar avg)
                ...
                if result3 and gate5:
                    ...
                    if os.getenv("DRY_RUN", "false").lower() != "true":
                        send_telegram_alert(
                            numbered,
                            symbol,
                            result3[2],  # LONG/SHORT
                            result3[3],  # price
                            "3min",
                            result3[5],  # score
                            result3[6]   # passed_required
                        )
                    last_sent[key3] = now
                    counter += 1
                ...
                if result5:
                    ...
                    if os.getenv("DRY_RUN", "false").lower() != "true":
                        send_telegram_alert(
                            numbered,
                            symbol,
                            result5[2],  # LONG/SHORT
                            result5[3],  # price
                            "5min",
                            result5[5],  # score
                            result5[6]   # passed_required
                        )
                    last_sent[key5] = now
                    counter += 1
