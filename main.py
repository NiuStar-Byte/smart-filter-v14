...
def run():
    while True:
        ...
        for symbol in TOKENS:
            # --- PHASE 1: 3-min double confirmation ---
            key3 = f"{symbol}_3min"
            df3 = fetch_ohlcv(symbol, "3min")
            if df3 is None:
                continue

            # If previously pending, send confirmed alert
            ...
            # If not pending, check initial pass and mark pending
            else:
                if not (key3 in last_sent and now - last_sent[key3] < COOLDOWN["3min"]):
                    sf3 = SmartFilter(symbol, df3, df3m=df3, df5m=None, tf="3min")
                    res3 = sf3.analyze()
            ...
            # --- PHASE 2: 5-min confirmation after 3-min confirmed ---
            key5 = f"{symbol}_5min"
